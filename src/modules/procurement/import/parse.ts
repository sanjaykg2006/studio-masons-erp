...
/** A quantity column: normally "Qty"/"Quantity", but some BOQs (interior /
 * furniture / civil) title it exactly "TOTAL" (total quantity). We match "total"
 * only when it's the whole header, so a "Total Amount" money column is NOT caught,
 * while "Total Qty" still matches via the words above. */
const isQtyHeader = (t: string) => hasWord(t, QTY) || norm(t) === "total";
...
function findHeader(ws: ExcelJS.Worksheet, scan = 20): { row: number; cols: FixedCols } | null {
  ...
  for (let r = 1; r <= last; r++) {
    ...
    for (let c = 1; c < vals.length; c++) {
      ...
      else if (!qty && isQtyHeader(t)) qty = c;
