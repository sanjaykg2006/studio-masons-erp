...
import { STUDIO_MASONS, page1Notes, annexure, DEFAULT_PAYMENT_TERMS, DEFAULT_NOTES } from "./po-terms";

/** A priced PO line the document renders. The HSN/SAC code is optional (only shown
 *  when captured). Mirrors the shape the old PoLinesEditor produced. */
export type PoLine = {
  service: string;
  unit: string;
  quantity: number;
