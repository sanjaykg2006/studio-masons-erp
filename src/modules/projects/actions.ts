...
import { renderBriefPdf } from "@/modules/projects/brief-pdf";
...
export async function createProject(
  ...
  await logAudit("project.create", `Created project "${trimmed}"`, { projectId: project.id });
...
export async function updateProject(
  ...
  await logAudit("project.update", "Updated a project", { projectId });
...
export async function deleteProject(projectId: string): Promise<ActionResult> {
  ...
  if (error) return fail(error.message);
  await logAudit("project.delete", "Deleted a project", { projectId });
  revalidatePath("/projects");
  return ok;
}
...
export async function finaliseProject(projectId: string): Promise<ActionResult> {
  ...
  const { error } = await supabase
    ...
    .eq("id", projectId);
  if (error) return fail(error.message);
  await logAudit("project.finalise", "Finalised a project", { projectId });
  revalidatePath(`/projects/${projectId}`);
  return ok;
}
...
export async function addMember(
  ...
  if (error) return fail(error.message);
  await logAudit("project.member.add", "Added a project member", { projectId, userId, roleId });
  revalidatePath(`/projects/${projectId}`);
  return ok;
}
...
export async function removeMember(projectId: string, userId: string): Promise<ActionResult> {
  ...
  if (error) return fail(error.message);
  await logAudit("project.member.remove", "Removed a project member", { projectId, userId });
  revalidatePath(`/projects/${projectId}`);
  return ok;
}
...
export async function createBriefs(
  ...
  await logAudit("project.brief.create", "Started project brief(s)", { projectId, templateIds });
  revalidatePath(`/projects/${projectId}`);
  return ok;
}
...
export async function submitBriefForReview(briefId: string): Promise<ActionResult> {
  ...
  const { error } = await supabase
    ...
    .eq("id", briefId);
  if (error) return fail(error.message);
  await logAudit("project.brief.submit", "Submitted a brief for review", { briefId });
  revalidatePath(`/projects/${brief.project_id}/brief/${briefId}`);
  return ok;
}
...
export async function uploadFile(
  ...
  await logAudit("project.folder.upload", "Uploaded a file", { projectId, folderKey, name: file.name });
...
export async function deleteFile(fileId: string): Promise<ActionResult> {
  ...
  await logAudit("project.folder.delete-file", "Deleted a file", {
...
export async function issueFiles(
  ...
  await logAudit("project.folder.issue", "Issued GFC files", { projectId, count: fileIds.length });
...
export async function raiseChangeRequest(
  ...
  if (error) return fail(error.message);
  await logAudit("project.change.raise", "Raised a change request", { projectId, title: t });
  revalidatePath(`/projects/${projectId}`);
  return ok;
}
