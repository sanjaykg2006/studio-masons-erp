-- Studio-Masons ERP — Procurement: fix approve_intent record-alias clash
-- Run AFTER 0045_intent_auto_amend.sql.
--
-- BUG  Approving an intent raised: record "il" is not assigned yet.
--   In approve_intent the FOR loop variable is named "il" and the table it reads
--   from was ALSO aliased "il". Inside the loop query, "il.budget_line_id" resolved
--   to the (still unassigned) loop record instead of the table column, aborting the
--   approval. Alias the table "l" so it no longer collides with the loop variable.
